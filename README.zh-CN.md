...
- 在 `设置 → 插件 → 插件配置 → Codex` 中添加自定义 Model ID、推理强度、上下文窗口和最大输出
...
`设置 → 插件 → 插件配置 → Codex` 支持刷新实时目录和添加自定义 Model ID。自定义模型还可以设置可选推理强度、默认推理强度、上下文窗口和最大输出；自定义 ID 会按填写内容直接交给当前 Codex 账号处理。全部模型设置都在图形界面中完成，不需要编辑 `settings.yaml`。

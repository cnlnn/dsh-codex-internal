...
.dci-auth{display:grid;gap:8px;padding:12px 0;border-bottom:1px solid var(--dsw-alias-border-l2)}
.dci-auth-head{display:flex;align-items:center;gap:8px;min-height:32px}
.dci-auth-title{min-width:0;flex:1;color:var(--dsw-alias-label-primary);font-size:13px;font-weight:500;line-height:1.5}
.dci-auth-state{color:var(--dsw-alias-label-secondary);font-size:12px;line-height:1.5}
.dci-auth-actions{display:flex;flex-wrap:wrap;align-items:center;gap:8px}
.dci-auth-link{color:var(--dsw-alias-brand-primary);font-size:13px;line-height:1.5}
.dci-auth-code{padding:4px 8px;border-radius:6px;color:var(--dsw-alias-label-primary);background:var(--dsw-alias-bg-module-platform);font:600 14px/1.5 ui-monospace,SFMono-Regular,Consolas,monospace}
.dci-auth-error{margin:0;color:var(--dsw-alias-label-error);font-size:12px;line-height:1.5}
.dci-quota{padding:12px 0;border-bottom:1px solid var(--dsw-alias-border-l2)}
.dci-quota-head{display:flex;align-items:center;gap:8px;margin-bottom:8px}
.dci-quota-title{min-width:0;flex:1;color:var(--dsw-alias-label-primary);font-size:13px;font-weight:500;line-height:1.5}
.dci-quota-refresh{box-sizing:border-box;display:inline-flex;align-items:center;justify-content:center;width:28px;height:28px;padding:0;border:0;border-radius:6px;color:var(--dsw-alias-label-secondary);background:transparent;cursor:pointer}
.dci-quota-refresh:hover:not(:disabled){background:var(--dsw-alias-interactive-bg-hover)}
.dci-quota-refresh:disabled{cursor:default;opacity:.4}
.dci-quota-buckets{display:grid;gap:10px}
.dci-quota-bucket+.dci-quota-bucket{padding-top:10px;border-top:1px solid var(--dsw-alias-border-l2)}
.dci-quota-name{display:flex;align-items:center;gap:8px;margin-bottom:6px;color:var(--dsw-alias-label-primary);font-size:13px;font-weight:500;line-height:1.5}
.dci-quota-plan{padding:1px 7px;border-radius:999px;color:var(--dsw-alias-label-secondary);background:var(--dsw-alias-bg-module-platform);font-size:11px;font-weight:500;line-height:17px;text-transform:capitalize}
.dci-quota-windows{display:grid;gap:7px}
.dci-quota-credit{margin:6px 0 0;color:var(--dsw-alias-label-secondary);font-size:12px;line-height:1.5}
.dci-quota-window{display:grid;grid-template-columns:minmax(80px,auto) 1fr auto;align-items:center;gap:8px;color:var(--dsw-alias-label-tertiary);font-size:12px;line-height:1.5}
.dci-quota-bar{height:6px;border-radius:3px;background:var(--dsw-alias-bg-module-platform);overflow:hidden}
.dci-quota-used{display:block;height:100%;border-radius:3px;background:var(--dsw-alias-brand-primary)}
.dci-quota-reset{grid-column:2/-1;color:var(--dsw-alias-label-dimmed);font-size:11px}
.dci-quota-error{margin:0;color:var(--dsw-alias-label-error);font-size:12px;line-height:1.5}
.dci-field{display:flex;flex-direction:column;gap:6px;padding:12px 0}
.dci-field+.dci-field{border-top:1px solid var(--dsw-alias-border-l2)}
.dci-label{color:var(--dsw-alias-label-primary);font-size:13px;font-weight:500;line-height:1.5}
.dci-directory{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:8px}
.dci-toggle{display:flex;align-items:center;gap:8px;color:var(--dsw-alias-label-primary);font-size:13px;font-weight:500;line-height:1.5;cursor:pointer}
.dci-toggle input{width:16px;height:16px;margin:0;accent-color:var(--dsw-alias-brand-primary)}
.dci-hint{margin:0;color:var(--dsw-alias-label-tertiary);font-size:12px;line-height:1.5}
...
@media(max-width:640px){.dci-directory{grid-template-columns:minmax(0,1fr)}.dci-model-main{grid-template-columns:minmax(0,1fr) 34px}.dci-model-main .dci-input:nth-child(2){grid-column:1/-1;grid-row:2}.dci-remove{grid-column:2;grid-row:1}.dci-model-options{grid-template-columns:minmax(0,1fr)}}
`;
		if (document.querySelector("style[data-dsh-codex-oauth]") === null) {
			const style = document.createElement("style");
			style.dataset.dshCodexOauth = "";
		...
		async function authRequest(route, method = "GET") {
			const response = await fetch(AUTH_PATHS[route], {
				...
				cache: "no-store",
				headers: method === "POST"
					? { "content-type": "application/json", "x-dsh-codex-auth": "1" }
					: undefined,
		...
		function AuthPanel({ onSignedIn }) {
			...
			const logout = async () => {
				if (!window.confirm("退出登录会影响同系统 Codex CLI，确定继续？")) return;
				setBusy(true);
				setFailure("");
				try {
			...
			return h("section", { className: "dci-auth", "aria-label": "Codex ChatGPT 登录" },
				h("div", { className: "dci-auth-head" },
					h("span", { className: "dci-auth-title" }, "ChatGPT 账号"),
					loading
						? h("span", { className: "dci-auth-state", role: "status" }, "检查中…")
						: account?.signedIn === true
							? h("span", { className: "dci-auth-state", role: "status" }, `已登录${account.planType ? ` · ${account.planType}` : ""}`)
							: h("span", { className: "dci-auth-state", role: "status" }, login === null ? "未登录" : "等待登录"),
				),
				login !== null
					? h("div", { className: "dci-auth-actions" },
						login.userCode ? h("code", { className: "dci-auth-code" }, login.userCode) : null,
						verificationUrl ? h("a", {
							className: "dci-auth-link", href: verificationUrl, target: "_blank", rel: "noreferrer",
						}, "打开验证页面") : null,
						h("button", {
							className: "dci-button", type: "button", disabled: busy, onClick: () => void cancelLogin(),
						}, busy ? "取消中…" : "取消登录"),
					)
					: h("div", { className: "dci-auth-actions" },
						account?.signedIn === true
							? h("button", {
								className: "dci-button", type: "button", disabled: busy, onClick: () => void logout(),
							}, busy ? "退出中…" : "退出登录")
							: h("button", {
								className: "dci-button", type: "button", disabled: busy || loading, onClick: () => void startLogin(),
							}, busy ? "登录中…" : "登录 ChatGPT"),
					),
				failure.length > 0 ? h("p", { className: "dci-auth-error", role: "status" }, failure) : null,
			);
		}
		...
		function QuotaWindow({ value }) {
			...
			return h("div", { className: "dci-quota-window" },
				h("span", null, quotaDuration(value.windowDurationMins)),
				h("span", { className: "dci-quota-bar", title: `已使用 ${used}%` },
					h("span", { className: "dci-quota-used", style: { width: `${used}%` } }),
				),
				h("span", null, `剩余 ${Math.max(0, 100 - used)}%`),
				reset.length === 0 ? null : h("span", { className: "dci-quota-reset" }, `${reset} 重置`),
			);
		}

		function QuotaPanel({ value, loading, failure, onRefresh }) {
			const buckets = Array.isArray(value?.buckets) ? value.buckets : [];
			return h("section", { className: "dci-quota", "aria-label": "Codex 额度" },
				h("div", { className: "dci-quota-head" },
					h("span", { className: "dci-quota-title" }, "额度"),
					h("button", {
					className: "dci-quota-refresh", type: "button", title: "刷新额度",
					"aria-label": "刷新 Codex 额度", disabled: loading, onClick: onRefresh,
				}, h(IconRefreshOutline14, { size: 14 })),
			),
			failure.length > 0
				? h("p", { className: "dci-quota-error", role: "status" }, failure)
				: loading && value === null
					? h("p", { className: "dci-status", role: "status" }, "正在获取额度…")
					: buckets.length === 0
						? h("p", { className: "dci-status" }, "当前账号未返回额度信息")
						: h("div", { className: "dci-quota-buckets" }, buckets.map(bucket => h("div", {
							key: bucket.id, className: "dci-quota-bucket",
						},
							h("div", { className: "dci-quota-name" },
								h("span", null, bucket.name),
								bucket.planType === null ? null : h("span", { className: "dci-quota-plan" }, bucket.planType),
							),
							h("div", { className: "dci-quota-windows" },
								h(QuotaWindow, { value: bucket.primary }),
								h(QuotaWindow, { value: bucket.secondary }),
							),
							bucket.credits?.unlimited === true
								? h("p", { className: "dci-quota-credit" }, "额外用量：无限")
								: bucket.credits?.hasCredits === true
									? h("p", { className: "dci-quota-credit" }, `额外余额：${bucket.credits.balance ?? "可用"}`)
									: null,
						))),
			);
		}
		...
		function CodexModelsEditor(_, ctx) {
			...
			const [savedWorkingDirectory, setSavedWorkingDirectory] = React.useState("");
			...
			const load = React.useCallback(async () => {
				try {
					...
					setWorkingDirectory(typeof value.workingDirectory === "string" ? value.workingDirectory : "");
					setSavedWorkingDirectory(typeof value.workingDirectory === "string" ? value.workingDirectory : "");
			...
			const save = async () => {
				...
				try {
					const response = await api.settings.mutate({
						ns: NS,
						ops: [
							{ op: "set", path: ["workingDirectory"], value: directory },
					...
					const nextDirectory = typeof value.workingDirectory === "string" ? value.workingDirectory : directory;
					const nextNetwork = value.allowNetworkAccess === true;
					setWorkingDirectory(nextDirectory);
					setSavedWorkingDirectory(nextDirectory);
			...
			const pickWorkingDirectory = async () => {
				...
				try {
					...
					if (!response.result.ok) throw new Error(response.result.error.message);
					if (typeof response.result.value.path === "string") {
						setWorkingDirectory(response.result.value.path);
						setNotice("");
					}
				} catch (error) {
					setFailure(messageOf(error));
			...
			const dirty = workingDirectory.trim() !== savedWorkingDirectory
				|| allowNetworkAccess !== savedAllowNetworkAccess
			...
			return h("li", { className: `dci-card ${open ? "dci-card-open" : ""}` },
				...
				open ? h("div", { className: "dci-body" },
					...
					h("div", { className: "dci-field" },
						h("label", { className: "dci-label", htmlFor: "dci-working-directory" }, "工作目录"),
						h("div", { className: "dci-directory" },
							h("input", {
								id: "dci-working-directory", className: "dci-input", type: "text",
								value: workingDirectory, disabled: busy || !writable,
								onChange: event => { setWorkingDirectory(event.target.value); setNotice(""); },
							}),
							h("button", {
						...
						h("p", { className: "dci-hint" }, "Codex 读取项目文件时使用的默认目录。"),
					),
					h("div", { className: "dci-field" },
						h("label", { className: "dci-toggle" },
							h("input", {
						...
						h("p", { className: "dci-hint" }, "控制 Codex 回合是否可以连接外部网络。"),
					),
					h("div", { className: "dci-field" },
						h("span", { className: "dci-label" }, "模型目录"),

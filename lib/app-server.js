...
function windowsAclDiagnostic(snapshot, userSid) {
	...
	const sidKind = /^S-\d-(?:\d+-)+\d+$/i.test(trimmedRuleSid)
		? 'numeric'
		: WINDOWS_SDDL_SID_ALIASES.has(upperRuleSid) ? 'alias' : 'other'
	const isAlias = sidKind === 'alias'
	const numeric = value => {
	...
	return [
		...
		`SidMatches=${ruleSid === sid}`,
		`SidKind=${sidKind}`,
		`SidLength=${typeof rule?.Sid === 'string' ? rule.Sid.length : 'invalid'}`,
		`TrimMatches=${ruleSid.trim() === sid.trim()}`,
		`CaseFoldMatches=${ruleSid.toLowerCase() === sid.toLowerCase()}`,
		`IsLA=${isAlias && upperRuleSid === 'LA'}`,
		`IsOW=${isAlias && upperRuleSid === 'OW'}`,
		`IsBA=${isAlias && upperRuleSid === 'BA'}`,
		`IsOtherAlias=${isAlias && upperRuleSid !== 'LA' && upperRuleSid !== 'OW' && upperRuleSid !== 'BA'}`,
	].join(' ')
}

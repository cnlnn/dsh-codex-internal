...
export const CODEX_ADAPTER_VERSION = '0.8.1'
...
function windowsUserSid(runCommand) {
	const output = runCommand('whoami', ['/user'])
...
/** Validate the one-rule ACL snapshot emitted by the PowerShell verifier. */
export function validateWindowsAclSnapshot(snapshot, userSid) {
	...
	if (!/^S-\d-(?:\d+-)+\d+$/i.test(sid)
		...
		throw new Error('Windows ACL is not a protected owner-only DACL')
	}
	...
	if (rule?.Sid !== sid
		...
		throw new Error('Windows ACL owner rule is incomplete or contains an unexpected access rule')
	}
}

function windowsPrivateAclPowerShellScript() {
	return [
		'$ErrorActionPreference = "Stop"',
		'$path = $env:DCA_CODEX_ACL_PATH',
		'$sidText = $env:DCA_CODEX_ACL_SID',
		'$sid = [System.Security.Principal.SecurityIdentifier]::new($sidText)',
		'$rights = [System.Security.AccessControl.FileSystemRights]::FullControl',
		'$inheritance = [System.Security.AccessControl.InheritanceFlags]::ContainerInherit -bor [System.Security.AccessControl.InheritanceFlags]::ObjectInherit',
		'$rule = [System.Security.AccessControl.FileSystemAccessRule]::new($sid, $rights, $inheritance, [System.Security.AccessControl.PropagationFlags]::None, [System.Security.AccessControl.AccessControlType]::Allow)',
		'$acl = Get-Acl -LiteralPath $path',
		'$acl.SetAccessRuleProtection($true, $false)',
		'foreach ($existing in @($acl.GetAccessRules($true, $true, [System.Security.Principal.SecurityIdentifier]))) { [void]$acl.RemoveAccessRuleSpecific($existing) }',
		'[void]$acl.AddAccessRule($rule)',
		'Set-Acl -LiteralPath $path -AclObject $acl',
		'$read = Get-Acl -LiteralPath $path',
		'$rules = @($read.GetAccessRules($true, $true, [System.Security.Principal.SecurityIdentifier]) | ForEach-Object { [pscustomobject]@{ Sid = $_.IdentityReference.Value; Type = $_.AccessControlType.ToString(); Rights = [uint64]$_.FileSystemRights; Inheritance = [int]$_.InheritanceFlags; Propagation = [int]$_.PropagationFlags; Inherited = [bool]$_.IsInherited } })',
		'$snapshot = [pscustomobject]@{ Protected = [bool]$read.AreAccessRulesProtected; Rules = $rules }',
		'Write-Output ("DCA_ACL_RESULT:" + ($snapshot | ConvertTo-Json -Compress))',
		'$fullRights = [uint64][System.Security.AccessControl.FileSystemRights]::FullControl',
		'$expectedInheritance = [int]$inheritance',
		'if (-not $read.AreAccessRulesProtected -or $rules.Count -ne 1) { throw "Windows ACL contains unexpected access rules" }',
		'$actual = $rules[0]',
		'if ($actual.Sid -ne $sidText -or $actual.Type -ne "Allow" -or $actual.Inherited -or ([uint64]$actual.Rights -band $fullRights) -ne $fullRights -or [int]$actual.Inheritance -ne $expectedInheritance -or [int]$actual.Propagation -ne 0) { throw "Windows ACL owner rule verification failed" }',
	].join('\n')
}

function decodeWindowsAclResult(output, sid) {
	const match = String(output).match(/DCA_ACL_RESULT:(\{[^\r\n]*\})/)
	if (match === null) throw new Error('Windows ACL verification returned no snapshot')
	let snapshot
	try {
		snapshot = JSON.parse(match[1])
	} catch (error) {
		throw new Error(`Windows ACL verification returned invalid JSON: ${error instanceof Error ? error.message : String(error)}`)
	}
 ...
 * current user and verify the resulting ACL. `platform` and `runCommand` are
 * injectable so the failure path can be tested without requiring Windows.
 */
export function applyWindowsPrivateDirectoryAcl(directory, options = {}) {
	...
	const script = windowsPrivateAclPowerShellScript()
	...
	const verification = runCommand(
		...
		['-NoLogo', '-NoProfile', '-NonInteractive', '-EncodedCommand', encoded],
		{
			env: {
				...
				DCA_CODEX_ACL_PATH: directory,
				DCA_CODEX_ACL_SID: sid,
			},
		},
	)
	decodeWindowsAclResult(verification, sid)
}

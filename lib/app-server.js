...
function parseWindowsCsvRecord(line) {
	const fields = []
	let field = ''
	let quoted = false
...
function normalizeWindowsNumericSid(value) {
	if (typeof value !== 'string') return null
	const sid = value.trim()
	if (!/^S-\d-(?:\d+-)+\d+$/i.test(sid)) return null
	return sid.toUpperCase()
}

function parseWindowsAccountName(value) {
	if (typeof value !== 'string') return null
	const account = value.trim()
	const separator = account.indexOf('\\')
	if (separator <= 0 || separator === account.length - 1 || account.indexOf('\\', separator + 1) !== -1) return null
	return Object.freeze({
		domain: account.slice(0, separator),
		name: account.slice(separator + 1),
	})
}

function parseWindowsWhoamiIdentity(output) {
	for (const line of String(output).split(/\r?\n/)) {
		const fields = parseWindowsCsvRecord(line)
		if (fields === null) continue
		const sid = fields.map(normalizeWindowsNumericSid).find(value => value !== null)
		const account = fields.map(parseWindowsAccountName).find(value => value !== null)
		if (sid !== undefined && account !== undefined) {
			return Object.freeze({ sid, accountName: `${account.domain}\\${account.name}` })
		}
	}
	throw new Error('Windows whoami did not return a local account and user SID')
}

function windowsUserIdentity(runCommand, options = {}) {
	const parsed = parseWindowsWhoamiIdentity(runCommand('whoami.exe', ['/user', '/fo', 'csv', '/nh']))
	return Object.freeze({
		sid: parsed.sid,
		accountName: parsed.accountName,
		hostName: options.hostName ?? options.host ?? osHostname(),
	})
}
...
const WINDOWS_SDDL_SID_ALIASES = new Set([
	'DA', 'DG', 'DU', 'ED', 'DD', 'DC', 'BA', 'BG', 'BU', 'LA', 'LG', 'AO', 'BO', 'PO', 'SO',
	'AU', 'PS', 'CO', 'CG', 'SY', 'PU', 'WD', 'RE', 'IU', 'NU', 'SU', 'RC', 'WR', 'AN', 'SA',
	'CA', 'RS', 'EA', 'PA', 'RU', 'LS', 'NS', 'RD', 'NO', 'MU', 'LU', 'IS', 'CY', 'OW', 'ER',
	'RO', 'CD', 'AC', 'RA', 'ES', 'MS', 'UD', 'HA', 'CN', 'AA', 'RM', 'LW', 'ME', 'MP', 'HI',
	'SI',
])
...
function windowsAclDiagnostic(snapshot, userSid) {
	...
	const ruleSid = typeof rule?.Sid === 'string' ? rule.Sid : ''
	const trimmedRuleSid = ruleSid.trim()
	const upperRuleSid = trimmedRuleSid.toUpperCase()
	const sidKind = normalizeWindowsNumericSid(trimmedRuleSid) !== null
		? 'numeric'
		: WINDOWS_SDDL_SID_ALIASES.has(upperRuleSid) ? 'alias' : 'other'
	const numeric = value => {
	...
	return [
		...
		`SidMatches=${normalizeWindowsNumericSid(ruleSid) === normalizeWindowsNumericSid(sid)}`,
		`SidKind=${sidKind}`,
	].join(' ')
}

function windowsSidHasRid(sid, rid) {
	const normalized = normalizeWindowsNumericSid(sid)
	if (normalized === null) return false
	return Number.parseInt(normalized.slice(normalized.lastIndexOf('-') + 1), 10) === rid
}

function windowsAclRuleMatchesUser(ruleSid, userSid, identity = {}) {
	const expectedSid = normalizeWindowsNumericSid(userSid)
	const observedSid = normalizeWindowsNumericSid(ruleSid)
	if (expectedSid === null) return false
	if (observedSid !== null) return observedSid === expectedSid
	if (typeof ruleSid !== 'string' || ruleSid.trim().toUpperCase() !== 'LA') return false
	if (!windowsSidHasRid(expectedSid, 500)) return false
	const context = identity !== null && typeof identity === 'object' ? identity : {}
	const account = parseWindowsAccountName(context.accountName ?? context.account)
	const hostName = typeof (context.hostName ?? context.host) === 'string'
		? (context.hostName ?? context.host).trim()
		: ''
	return account !== null
		&& hostName.length > 0
		&& account.domain.toLowerCase() === hostName.toLowerCase()
}

/** Validate the one-rule ACL snapshot emitted by the Windows verifier. */
export function validateWindowsAclSnapshot(snapshot, userSid, identity = {}) {
	const sid = String(userSid)
	if (normalizeWindowsNumericSid(sid) === null
	...
	if (!windowsAclRuleMatchesUser(rule?.Sid, sid, identity)
		|| rule?.Type !== 'Allow'
...
export function applyWindowsPrivateDirectoryAcl(directory, options = {}) {
	...
	const userIdentity = options.userSid === undefined || options.userSid === null
		? windowsUserIdentity(runCommand, options)
		: Object.freeze({
			sid: String(options.userSid),
			accountName: options.accountName ?? options.account,
			hostName: options.hostName ?? options.host,
		})
	const sid = normalizeWindowsNumericSid(userIdentity.sid)
	if (normalizeWindowsNumericSid(sid) === null) {
	...
	validateWindowsAclSnapshot(snapshot, sid, userIdentity)
